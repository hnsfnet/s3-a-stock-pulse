...
class BaseRepository:
    ...
    def _execute(self, query, params=None):
        ...
        log_sql(self.logger, query, params)
        if params:
    ...
    def _executemany(self, query, seq_params):
        cursor = self.db.cursor()
        log_sql(self.logger, f"{query} (batch of {len(seq_params)})")
...
class CategoryRepository(BaseRepository):
    ...
    def get_subcategories(self, parent_id):
        query = 'SELECT * FROM categories WHERE parent_id = ? ORDER BY id'
        cursor = self._execute(query, (parent_id,))
        return self._to_list(cursor)
...
class AccountRepository(BaseRepository):
    def get_all(self):
        cursor = self._execute('SELECT * FROM accounts ORDER BY id')
        return self._to_list(cursor)

    def get(self, account_id):
        cursor = self._execute('SELECT * FROM accounts WHERE id=?', (account_id,))
    ...
    def update(self, account_id, name=None, type_=None, balance=None):
        ...
        cursor = self._execute(f'UPDATE accounts SET {", ".join(fields)} WHERE id=?', params)
        self.db.commit()
    ...
    def delete(self, account_id):
        ...
        if cursor.fetchone()['c'] > 0:
            return False
        cursor = self._execute('DELETE FROM accounts WHERE id=?', (account_id,))
        self.db.commit()
        return cursor.rowcount > 0
...
class TransactionRepository(BaseRepository):
    ...
    def delete(self, txn_id):
        cursor = self._execute('DELETE FROM transactions WHERE id=?', (txn_id,))
        self.db.commit()
        return cursor.rowcount > 0
...
class BudgetRepository(BaseRepository):
    ...
    def delete(self, budget_id):
        cursor = self._execute('DELETE FROM budgets WHERE id=?', (budget_id,))
        self.db.commit()
        return cursor.rowcount > 0
...
class RecurringRuleRepository(BaseRepository):
    ...
    def get_all(self, active_only=False):
        ...
        '''
        if active_only:
            cursor = self._execute(base_sql + ' WHERE r.active = 1 ORDER BY r.next_date')
        else:
            cursor = self._execute(base_sql + ' ORDER BY r.active DESC, r.next_date')
        return self._to_list(cursor)

    def get(self, rule_id):
        cursor = self._execute('SELECT * FROM recurring_rules WHERE id=?', (rule_id,))
        return self._to_one(cursor)
    ...
    def update(self, rule_id, name=None, frequency=None, interval_val=None,
        ...
        cursor = self._execute(f'UPDATE recurring_rules SET {", ".join(fields)} WHERE id=?', params)
        self.db.commit()
        return cursor.rowcount > 0

    def update_next_date(self, rule_id, next_date):
        next_date = DatabaseConnection.normalize_date(next_date)
        cursor = self._execute('UPDATE recurring_rules SET next_date=? WHERE id=?', (next_date, rule_id))
        self.db.commit()

    def deactivate(self, rule_id):
        cursor = self._execute('UPDATE recurring_rules SET active=0 WHERE id=?', (rule_id,))
        self.db.commit()

    def delete(self, rule_id):
        cursor = self._execute('DELETE FROM recurring_rules WHERE id=?', (rule_id,))
        self.db.commit()
        return cursor.rowcount > 0
